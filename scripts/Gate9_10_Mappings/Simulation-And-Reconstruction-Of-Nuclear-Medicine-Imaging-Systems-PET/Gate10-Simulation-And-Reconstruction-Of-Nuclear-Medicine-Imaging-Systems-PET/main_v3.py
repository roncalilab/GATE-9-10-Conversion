...
Translated from GATE 9 main.mac with coincidence processing
"""
...
def main():
    ...
    # =========================================================================
    # DIGITIZER CHAIN (from pet_digitizer.mac)
    # =========================================================================
    # Hits collection
    ...
    hits.write_to_disk = False  # Don't need to write intermediate steps
    hits.attributes = [
        "TotalEnergyDeposit",
        "PostPosition",
        "PrePosition",
    ...
    energy_blur.write_to_disk = False
    
    # Energy window - this will be our "Singles" output
    energy_window = sim.add_actor("DigitizerEnergyWindowsActor", "Singles_LSO")
    ...
    energy_window.output_filename = "singles.root"  # Separate file first
    energy_window.write_to_disk = True
    
    # Note: Coincidence processing will be done in post-processing
    # because GATE 10 doesn't have DigitizerCoincidenceSorterActor yet
    
    print("\n=== Digitizer Chain Summary ===")
    print(f"Hits -> {hits.name}")
    print(f"Singles (Adder) -> {singles.name}")
    print(f"Energy Blur -> {energy_blur.name}")
    print(f"Energy Window -> {energy_window.name}")
    print("================================\n")
